...
export default function GroupModal({ isOpen, onClose, onSave }: GroupModalProps) {
    ...
    const [studentsOptions, setStudentsOptions] = useState<Student[]>([]);
        ...
        if (!isAddTeacherModalOpen) {
            if (teachersOptions.length === 0) {
                api.get('/teachers').then(res => {
                    const list: Teacher[] = res.data?.data || res.data || [];
    ...
    const handleDayCheckboxChange = (dayId: string) => {
        setGroupData(prev => {
    ...
    const fetchCourses = () => {
        ...
        api.get('/courses')
            .then(res => {
                const coursesList: Course[] = res.data?.data || res.data || [];
    ...
    const fetchRooms = () => {
        ...
        api.get('/rooms')
            .then(res => {
                const roomsList: Room[] = res.data?.data || res.data || [];
                setRooms(roomsList);
            })
            .catch(err => {
    ...
    const handleSubmit = (e?: React.FormEvent) => {
        ...
        let formattedDate = startDate as string;
        // Parse and convert dd/mm/yyyy or dd.mm.yyyy to YYYY-MM-DD ISO format automatically
        const ddmmyyyyRegex = /^(\d{1,2})[./-](\d{1,2})[./-](\d{4})$/;
        const match = (startDate as string).match(ddmmyyyyRegex);
        if (match) {
