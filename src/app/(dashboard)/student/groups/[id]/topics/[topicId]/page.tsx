...
import { useTopicDetail } from '../topic-context';
...
export default function TopicDetailPage() {
    ...
    const { expandedTopicId, currentTopic, videos, selectedVideo } = useTopicDetail();
    ...
    const homeworkFetchedForTopicRef = useRef<number | null>(null);
    useEffect(() => {
        if (expandedTopicId && homeworkFetchedForTopicRef.current !== expandedTopicId) {
            homeworkFetchedForTopicRef.current = expandedTopicId;
            ...
            api.get(`/groups/${groupId}/lessons/${expandedTopicId}/homeworks`)
                ...
                .catch(err => {
                    ...
                    homeworkFetchedForTopicRef.current = null;
                });
        }
    ...
    return (
        <div className={styles.contentBody}>
            <div className={`${styles.videoWrapper} ${!selectedVideo ? styles.videoWrapperEmpty : ""}`}>
                {selectedVideo ? (
                    <video
            ...
            )) : currentTopic && !(currentTopic.videoCount > 0) && (
